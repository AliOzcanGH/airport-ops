...
class JwtSignatureVerificationIntegrationTests {
    ...
    void acceptsTokenSignedByThePublishedKey() throws Exception {
        ...
        ResponseEntity<String> response = restTemplate.exchange(
                "/organizations/" + organizationId + "/reports/daily-flights?date=" + LocalDate.now(),
                HttpMethod.GET,
                new HttpEntity<>(headers),
                String.class);

        // Signature verifies; a freshly-generated org simply has no report data.
    ...
    private void assertUnauthorized(String token) {
        ...
        headers.setBearerAuth(token);
        ResponseEntity<String> response = restTemplate.exchange(
                "/organizations/" + UUID.randomUUID() + "/reports/daily-flights?date=" + LocalDate.now(),
                HttpMethod.GET,
                new HttpEntity<>(headers),
                String.class);
