package com.aliozcan.airportops.report_service.testsupport;
...
public class TestIamJwtDecoderConfig {
    ...
    public static final String ORG_A_VIEWER_NO_PERMISSION_TOKEN = "org-a-viewer-no-permission-token";

    @Bean
    @Primary
    public JwtDecoder testJwtDecoder() {
        return token -> switch (token) {
            case ORG_A_ADMIN_TOKEN -> tenantJwt(token, ORG_A, List.of("AIRLINE_ADMIN"), List.of("report:read"));
            case ORG_B_ADMIN_TOKEN -> tenantJwt(token, ORG_B, List.of("AIRLINE_ADMIN"), List.of("report:read"));
            case ORG_A_VIEWER_NO_PERMISSION_TOKEN -> tenantJwt(token, ORG_A, List.of("VIEWER"), List.of());
    ...
    private Jwt tenantJwt(String token, UUID organizationId, List<String> roles, List<String> permissions) {
        ...
        return Jwt.withTokenValue(token)
                ...
                .audience(List.of("report-service"))
                .issuedAt(issuedAt)
