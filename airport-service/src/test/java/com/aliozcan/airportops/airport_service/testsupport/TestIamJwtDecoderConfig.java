package com.aliozcan.airportops.airport_service.testsupport;
...
public class TestIamJwtDecoderConfig {
    ...
    public JwtDecoder testJwtDecoder() {
        return token -> switch (token) {
            case ADMIN_TOKEN -> jwt(token, ORG_A, List.of("AIRLINE_ADMIN"),
                    List.of("station:create", "station:read", "gate:read", "gate:update"));
            case VIEWER_TOKEN -> jwt(token, ORG_A, List.of("VIEWER"),
                    List.of("station:read", "gate:read"));
    ...
    private Jwt jwt(String token, UUID organizationId, List<String> roles, List<String> permissions) {
        ...
        return Jwt.withTokenValue(token)
                ...
                .audience(List.of("airport-service"))
                .issuedAt(issuedAt)
